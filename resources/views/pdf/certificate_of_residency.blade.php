...
<head>
    ...
    <style>
        ...
        /* Footer meta */
        .meta { margin-top: 10mm; font-size: 12px; }
        .meta td { padding: 2mm 4mm 0 0; }

        /* Disclaimer */
        .disclaimer { margin-top: 8mm; font-size: 11px; font-style: italic; opacity: 0.8; }
    </style>
</head>
<body>
@php
    ...
    $dob = $certificate['applicant_profile']['date_of_birth'] ?? null;
    $age = $dob ? \Carbon\Carbon::parse($dob)->age : null;
    $gender = strtolower($certificate['applicant_profile']['gender'] ?? '');
    $pronoun = $gender === 'male' ? 'he' : ($gender === 'female' ? 'she' : 'he/she');

    $issueDate = $certificate['application_date'] ?? null;
...
<div class="page">
    ...
    <p class="paragraph">
        ...
        @if($age) , <strong>{{ $age }}</strong> years old @endif
        and a resident of Barangay {{ $barangay }}, {{ $city }}, {{ $province }}, is known to be a bonafide resident of this community.
    </p>
    <p class="paragraph">
        Further certify that {{ $pronoun }} has been recorded as a resident of the above address in the official barangay records.
    </p>
    <p class="paragraph">
        <strong>ISSUED</strong> this <strong>{{ $issuedDay }}</strong> day of <strong>{{ $issuedMonth }}</strong>,
        <strong>{{ $issuedYear }}</strong> at Barangay {{ $barangay }}, {{ $city }}, {{ $province }}, upon request of the interested party for whatever legal purposes it may serve.
    </p>
    ...
    <div class="disclaimer">
        Note: This certificate is not verified if no official stamp and signature are present.
    </div>

    <table class="meta">
        <tr>
            <td>Residency No.: #{{ $certificate['id'] ?? '____________________' }}</td>
