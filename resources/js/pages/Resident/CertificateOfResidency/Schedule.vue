...
const props = defineProps<{ residency: { id: number; status: string; application_date?: string | null; appointment_at?: string | null; appointment_status?: string | null }, rescheduleAllowed?: boolean }>()

const form = useForm({
  residency_id: props.residency.id,
  date: '',
  time: ''
})
...
async function loadOccupied(date: string) {
  try {
    const base = route('resident.certificate-of-residency.availability')
...
const appointmentDisplay = computed(() => {
  ...
  try {
    const d = new Date(props.residency.appointment_at)
    return d.toLocaleString(undefined, { dateStyle: 'medium', timeStyle: 'short', hour12: true })
  } catch { return props.residency.appointment_at }
})
...
function submit() {
  ...
  form.post(route('resident.certificate-of-residency.schedule.store'), {
    preserveScroll: true,
    onSuccess: () => {
      form.reset()
    }
...
const breadcrumbs = [
  { title: 'Certificate of Residency', href: route('resident.certificate-of-residency.create') },
  { title: 'Schedule Pickup', href: route('resident.certificate-of-residency.schedule') },
]
</script>
          ...
          <CardHeader>
            <div class="flex items-center justify-between">
              <div>
                ...
                <CardDescription>Pick a date and time to claim your certificate.</CardDescription>
              </div>
              <img src="/images/thankyou.png" alt="Certificate of Residency" class="hidden sm:block h-12 w-auto rounded-md" />
            </div>
          </CardHeader>
          <CardContent class="space-y-6 text-[#2C4854]">
            <div class="rounded border border-neutral-200 bg-white p-4">
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <div class="text-xs opacity-70">Residency ID</div>
                  <div class="text-sm">#{{ props.residency.id }}</div>
                </div>
                <div>
                  <div class="text-xs opacity-70">Status</div>
                  <div class="text-sm font-medium">{{ props.residency.status }}</div>
                </div>
                <div v-if="props.residency.application_date">
                  <div class="text-xs opacity-70">Application Date</div>
                  <div class="text-sm">{{ props.residency.application_date }}</div>
                </div>
                <div v-if="appointmentDisplay">
                  <div class="text-xs opacity-70">Current Appointment</div>
                  <div class="text-sm">{{ appointmentDisplay }}</div>
                </div>
                <div v-if="props.residency.appointment_status">
                  <div class="text-xs opacity-70">Appointment Status</div>
                  <div class="text-sm capitalize">{{ (props.residency.appointment_status as string).replace('_', ' ') }}</div>
                </div>
              </div>
            </div>
            ...
            <form @submit.prevent="submit" class="space-y-4">
              ...
              <input type="hidden" name="residency_id" :value="form.residency_id" />
              ...
              <div class="flex items-center gap-2">
                ...
                <Link :href="route('resident.certificate-of-residency.create')">
