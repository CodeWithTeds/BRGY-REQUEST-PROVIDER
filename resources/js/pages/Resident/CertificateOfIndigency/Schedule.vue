...
async function loadOccupied(date: string) {
  try {
    const base = route('resident.certificate-of-indigency.availability')
...
const appointmentDisplay = computed(() => {
  ...
  try {
    const d = new Date(props.indigency.appointment_at)
    return d.toLocaleString(undefined, { dateStyle: 'medium', timeStyle: 'short', hour12: true })
  } catch { return props.indigency.appointment_at }
})
...
function submit() {
  ...
  form.post(route('resident.certificate-of-indigency.schedule.store'), {
    preserveScroll: true,
    onSuccess: () => {
      form.reset()
    }
...
const breadcrumbs = [
  { title: 'Certificate of Indigency', href: route('resident.certificate-of-indigency.create') },
  { title: 'Schedule Pickup', href: route('resident.certificate-of-indigency.schedule') },
]
</script>
        ...
        <Card>
          <CardHeader>
            <div class="flex items-center justify-between">
              ...
              <img src="/images/thankyou.png" alt="Certificate of Indigency" class="hidden sm:block h-12 w-auto rounded-md" />
            </div>
          </CardHeader>
          <CardContent class="space-y-6 text-[#2C4854]">
            <div class="rounded border border-neutral-200 bg-white p-4">
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <div class="text-xs opacity-70">Indigency ID</div>
                  <div class="text-sm">#{{ props.indigency.id }}</div>
                </div>
                <div>
                  <div class="text-xs opacity-70">Status</div>
                  <div class="text-sm font-medium">{{ props.indigency.status }}</div>
                </div>
                <div v-if="props.indigency.application_date">
                  <div class="text-xs opacity-70">Application Date</div>
                  <div class="text-sm">{{ props.indigency.application_date }}</div>
                </div>
            ...
            <form @submit.prevent="submit" class="space-y-4">
              ...
              <input type="hidden" name="indigency_id" :value="form.indigency_id" />

              <div class="flex items-center gap-2">
                <Button type="submit" :disabled="props.rescheduleAllowed === false && !!appointmentDisplay">Save Appointment</Button>
                <Link :href="route('resident.certificate-of-indigency.create')">
