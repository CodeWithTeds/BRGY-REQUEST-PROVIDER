...
const props = defineProps<{ permit: { id: number; status: string; application_date?: string | null; appointment_at?: string | null; appointment_status?: string | null }, rescheduleAllowed?: boolean }>()
...
async function loadOccupied(date: string) {
  try {
    const base = route('barangay-permit.availability')
...
const appointmentDisplay = computed(() => {
  ...
  try {
    const d = new Date(props.permit.appointment_at)
    return d.toLocaleString(undefined, { dateStyle: 'medium', timeStyle: 'short', hour12: true })
  } catch { return props.permit.appointment_at }
})

function submit() {
  if (props.rescheduleAllowed === false && appointmentDisplay.value) {
    // Prevent submission if reschedule limit reached and an appointment already exists
    return;
  }
  form.post(route('barangay-permit.schedule.store'), {
    preserveScroll: true,
    onSuccess: () => {
      form.reset()
    }
...
const breadcrumbs = [
  { title: 'Barangay Business Permit', href: route('resident.barangay-business-permit') },
  { title: 'Schedule Pickup', href: route('barangay-permit.schedule') },
]
</script>
          ...
          <CardHeader>
            <div class="flex items-center justify-between">
              <div>
                ...
                <CardDescription>Pick a date and time to claim your permit.</CardDescription>
              </div>
              <img src="/images/business.png" alt="Business Permit" class="hidden sm:block h-12 w-auto rounded-md" />
            </div>
          </CardHeader>
          <CardContent class="space-y-6 text-[#2C4854]">
            <div class="rounded border border-neutral-200 bg-white p-4">
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <div class="text-xs opacity-70">Permit ID</div>
                  <div class="text-sm">#{{ props.permit.id }}</div>
                </div>
                <div>
                  <div class="text-xs opacity-70">Status</div>
                  <div class="text-sm font-medium">{{ props.permit.status }}</div>
                </div>
                <div v-if="props.permit.application_date">
                  <div class="text-xs opacity-70">Application Date</div>
                  <div class="text-sm">{{ props.permit.application_date }}</div>
                </div>
                <div v-if="appointmentDisplay">
                  <div class="text-xs opacity-70">Current Appointment</div>
                  <div class="text-sm">{{ appointmentDisplay }}</div>
                </div>
                <div v-if="props.permit.appointment_status">
                  <div class="text-xs opacity-70">Appointment Status</div>
                  <div class="text-sm capitalize">{{ (props.permit.appointment_status as string).replace('_', ' ') }}</div>
                </div>
              </div>
            </div>
            ...
            <div class="rounded border border-neutral-200 bg-white p-4">
              ...
              <ul class="mt-2 pl-5 space-y-1 list-disc text-sm">
                ...
                <li>Your reference number: <span class="font-medium">{{ referenceNo }}</span> (present this at the counter).</li>
              ...
              <div class="mt-3 text-sm">
                Office hours: <span class="font-medium">Monday–Friday, 8:00 AM–5:00 PM</span>. Walk-ins may not be accepted; please arrive at your scheduled time.
              </div>
            ...
            <form @submit.prevent="submit" class="space-y-4">
              ...
              <input type="hidden" name="permit_id" :value="form.permit_id" />
