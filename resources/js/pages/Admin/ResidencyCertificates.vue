...
interface ResidencyItem {
  ...
  status: 'approved' | 'pending' | 'processing' | 'pre-approved' | 'rejected' | string;
...
function deleteResidency(id: number) {
  if (!confirm('Delete this Residency Certificate?')) return;
  router.delete(`${basePath.value}/${id}`, {
    onSuccess: () => {
      notify('Deleted Residency Certificate!', 'success');
    },
    onError: () => {
...
<template>
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <template #header>
      <div class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-4">
          <button class="inline-flex items-center gap-2 px-3 py-2 bg-[#2c4454] text-white text-sm rounded-md hover:opacity-90">
            <span class="text-white">+ Add new certificate</span>
          </button>
      ...
      <div class="mt-2 text-lg font-semibold text-[#2c4454]">Certificate of Residency</div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody class="bg-white divide-y divide-gray-200">
                    <tr v-for="item in paginatedResidencies" :key="item.id" class="hover:bg-gray-50">
                      ...
                      <td class="px-3 py-2 whitespace-nowrap">
                        <div class="flex items-center gap-1">
                          <span class="h-2 w-2 rounded-full" :class="{
                            ...
                            'bg-indigo-500': item.status === 'pre-approved',
                            'bg-red-500': item.status === 'rejected',
                          }" />
                          <span class="text-xs text-[#2c4454] capitalize">{{ item.status }}</span>
                        </div>
                      </td>
                      <td class="px-6 py-4 whitespace-nowrap text-sm text-[#2c4454] opacity-80">{{ item.application_date || '—' }}</td>
                      ...
                      <td class="px-6 py-4 whitespace-nowrap text-sm text-[#2c4454] opacity-80">{{ item.remarks || '—' }}</td>
                      <td class="px-6 py-4 whitespace-nowrap text-sm text-[#2c4454] opacity-80">{{ item.updated_at || '—' }}</td>
